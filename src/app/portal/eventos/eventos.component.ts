...
export class EventosComponent implements OnInit { 
	...
	excel () {
		...
		this.service.getEventos(this.convertFecha (this.fechaDesde), this.convertFecha (this.fechaHasta), this.usuario, this.canal, this.accion, null, true).subscribe( response => {
            ...
            this.excelService.exportAsExcelFile(this.export,null, 'eventos');
	  ...
	  buscar () {
		...
		this.buscaron = false;
		  ...
		  if (this.fechaDesde == null || this.fechaDesde == "") {
			...
			(this.usuario.substring(0,1)==" " ||  this.usuario.substring( this.usuario.length-1, this.usuario.length)==" ")){

				this.entorno.pivot_msg.act = true;

				this.entorno.pivot_msg.type = 'alert-danger';

				this.entorno.pivot_msg.mensaje ="El nombre de usuario no debe contener espacios al principio ni al final, tampoco más de un espacio entre palabras. Por favor elimine el espacio del final";
				this.loading = false;
				this.entorno.hideAlert();
		  	}  else {
